		...
		genTree:function(options) {
			...
			return this.each(function(){
				...
				if (tree.size()>0) {
					...
					$(">div", node).prepend("<div class='" + ((op.showSub && !op.ckbox) ? op.coll : op.exp) + "'></div>"+(op.ckbox ?"<div class='ckbox " + checked + "'></div>":"")+(op.icon?"<div class='"+ ((op.showSub && !op.ckbox) ? op.options.folderColl : op.options.folderExp) +"'></div>":""));
					(op.showSub && !op.ckbox) ? tree.show() : tree.hide();
					...
					if(op.showSub || op.ckbox) tree.subTree(op, op.level + 1);
				} else {
...
//			var url = $this.attr("href");
		...
		return this.each(function(){
			...
			$this.click(function(event){
				...
				if (title) {
					alertMsg.confirm(title, {
						okCall: function(){_doExport($this, url);}
					});
				} else {_doExport($this, url);}
