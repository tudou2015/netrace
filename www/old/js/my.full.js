...
{
		productMode = false;
    if($.cookie) productMode = $.cookie('productMode');
...
{
    if($.cookie && $('#productID')) $.cookie('lastProduct', $('#productID').val(), {expires:config.cookieLife, path:config.webRoot});
}
...
{
		projectMode = false;
    if($.cookie) projectMode = $.cookie('projectMode');
...
{
    if($.cookie && $('#projectID')) $.cookie('lastProject', $('#projectID').val(), {expires:config.cookieLife, path:config.webRoot});
}
...
{
    if($.cookie && !$.cookie('help')) $.cookie('help', 'off', {expires:config.cookieLife, path:config.webRoot});
    className = $.cookie && $.cookie('help') == 'off' ? 'hidden' : '';
...
function saveWindowSize()
{
    ...
    if($.coookie) $.cookie('windowWidth',  width)
    if($.coookie) $.cookie('windowHeight', height)
}
...
{  
    if($.browser && $.browser.msie && $('.debugwin').size() == 1)
    {
