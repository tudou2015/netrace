...
class html
{
     ...
     * @return string
     */
    static public function select($name = '', $options = array(), $selectedItems = "", $attrib = "", $default = '')
    {
    ...
    static public function radio($name = '', $options = array(), $checked = '', $attrib = '')
    {
        ...
        foreach($options as $key => $value)
        {
            ...
            $string .= ($checked == ",$key,") ? " checked ='checked'" : "";
            $string .= $attrib;
            $string .= " /> $value\n";
        }
        return $string;
    ...
    static public function checkbox($name, $options, $checked = "", $attrib = "")
    {
        ...
        foreach($options as $key => $value)
        {
            ...
            $string .= $attrib;
            $string .= " /> <label>$value</label></span><br />\n";
        }
        return $string;
    }
...
class js
{
   /**
    ...
    {
        if($full) return "<meta http-equiv='Content-Type' content='text/html; charset=utf-8' /><script language='Javascript'>";
    ...
    {
        return self::start() . "alert('" . strtr($message, "\r\n'\"", '????') . "')" . self::end() . self::resetForm();
    }
...
EOT;
     ...
     */
    static public function locate($url, $target = "self", $debug = false)
    {
    	if($debug)
    	{
    		global $lang;
        ...
        else
        {
    		...
    		return html::commonButton($lang->reload, "onclick='$js'");
    	}
			else
			{    	
     ...
     * @return  string the javascript string.
     */
    static public function reload($window = 'self', $debug = false)
    {
    	if($debug)
    	{
    		global $lang;
    		return html::commonButton($lang->reload, "onclick='$window.location.href=$window.location.href'");
    	}
    	else
    	{
        $js  = self::start();
