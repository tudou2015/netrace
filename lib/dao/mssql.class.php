...
class dao
{
    ...
    public function datas($datas)
    {
        if(!is_array($datas)) 
        {
					$this->app->error('Must be array', __FILE__, __LINE__, $exit = true);        	
        }
        if(!count($datas)) 
        {
					$this->app->error('Array is empty', __FILE__, __LINE__, $exit = true);        	
        }
    ...
    {    	
        ...
        { 
        	echo 'dao::query'; 
        	a(dao::$errors); 
					$this->app->error(var_export(dao::$errors, true),  __FILE__, __LINE__, $exit = true); 
	        exit; 
    ...
    public function autoCheck($skipFields = '')
    {
        ...
        $fields     = $this->getFieldsType();//Array ( [id] => Array ( [rule] => int ) [org] => Array ( [rule] => int ) [name] => Array ( [rule] => length [options] => Array ( [max] => 50 [min] => 0 ) ) [password] => Array ( [rule] => length [options] => Array ( [max] => 50 [min] => 0 ) ) [visit] => Array ( [rule] => int ) [ip] => Array ( [rule] => length [options] => Array ( [max] => 50 [min] => 0 ) ) [last] => Array ( [rule] => int ) [deleted] => Array ( [rule] => reg [options] => Array ( [reg] => /0|1/ ) ) ) 
...
class sql
{
    ...
    public function idata($data)
    {
        $this->data = $data;
        $this->sql .= '(';
        foreach($data as $field => $value) $this->sql .= " $field ,";
    ...
    {    		
        ...
        //必须是array
        if(!is_array($limit)) die("[$sql]Limit error in file:".__FILE__.',line:'.__LINE__);
				
				$sql = $this->sql;		
				//不是select		
        if(strpos($sql, 'SELECT') === false) die("[$sql]Limit error in file:".__FILE__.',line:'.__LINE__);       
        ...
        if($orderPOS) 
        {
        	...
        	$sql = substr($sql, 0, $orderPOS);//去掉order by
	        //多个order by ,无法处理
  	      //if(strchr($order, ',') !== false)  die("[$sql]Limit error in file:".__FILE__.',line:'.__LINE__);         	      
    		}
    		else
    		{
    			$order =  'order by id';   
        ...
        $sql = str_replace('SELECT', "SELECT row_number() over($order) rowid,", $sql);
        
       	$sql = "select * from ($sql) a where rowid between {$limit[0]} and {$limit[1]}";
